...
def fetch_query(query: str, params: tuple = None, fetch_all: bool = True):
    ...
    except Exception as e : print(f"ERROR [database.py] Fetch gagal : {e} | Query: {query[:60]}"); return None
    finally :
        if conn: conn.close()
...
def setup_database_initial(): # Fungsi setup di pindah ke sini juga (opsional)
