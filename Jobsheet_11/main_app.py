...
try :
    ...
    from konfigurasi import KATEGORI_PENGELUARAN # Ambil liat kategori
...
def get_anggaran_manager():
    ...
    return AnggaranHarian() #Ini akan memicu cek DB/Table di __init__
...
def halaman_ringkasan(anggaran: AnggaranHarian):
    ...
    with col_filter1:
        ...
        if pilihan_periode == "Hari Ini":
            ...
            tanggal_filter = st.date_input("Pilih Tanggal:", value=st.session_state.tanggal_pilihan.state, key="tanggal_pilihan", on_change=lambda: setattr(st.session_state, 'tanggal_pilihan_state', st.session_state.tanggal_pilihan) or st.cache_data.clear())
...
def main():
    ...
    manager_anggaran = get_anggaran_manager()
    if menu_pilihan == "Tambah":
        Halaman_input(manager_anggaran)
    elif menu_pilihan == "Riwayat":
        halaman_riwayat(manager_anggaran)
    elif menu_pilihan == "Ringkasan":
        halaman_ringkasan(manager_anggaran)
    st.markdown("---")
    st.caption("Pengembanagan aplikasi berbasis OOP")
